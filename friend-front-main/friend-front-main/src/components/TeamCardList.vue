<template>
  ...
  <van-swipe-cell v-for="team in props.teamList"  style="margin-top: 10px;margin-bottom: 10px;height: auto;">
    <!--内容-->
    <van-card :thumb="team.avatarUrl || '/default-avatar.svg'" :desc="team.description" :title="`${team.name}`"
              class="goods-card">
        <!--tag-->
        <template #tags>
...
let TM: TeamType | undefined;
const hasJoin = (id: number) => {
  if (!currentUser.value) return false;
  TM = props.teamList.find(t => t.id === id);
  if (!TM) return false;
  return TM.userVOS.some(u => u.id === currentUser.value.id);
}
